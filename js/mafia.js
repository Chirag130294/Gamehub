...
const MafiaEngine = {
 ...
 gameState: {
        ...
        winnerTitle: null, winnerMessage: null, winnerIcon: null, winnerColor: null, isPaused: false
    },
    ...
    connectMQTT(mode, btnElement = null) {
        ...
        this.mqttClient.on('connect', () => {
            if (this.isHost) {
                ...
                // --- Activate the Universal Header Room Code ---
                document.getElementById('header-room-code-val').innerText = this.roomId;
                ...
                if (isFirstConnect && mode === 'PLAYER_NEW') {
                    ...
                    if(typeof GujaratLoader !== 'undefined') GujaratLoader.start(); // FIXED: Start loader immediately!
                } else {
...
renderHostLobby() {
        this.showScreen('mafia-screen-lobby');
        if(typeof GujaratLoader !== 'undefined') GujaratLoader.start();
        ...
        this.broadcastState(); // ALWAYS broadcast state so waiting clients sync up
    },

 handleHostReceivesData(peerId, msg) {
        if (msg.type === 'SPECTATOR_GUESS') {
            if (this.gameState.players[peerId]) {
                // Initialize the tracking array if it doesn't exist
                if (!this.gameState.players[peerId].stats.spectatorGuesses) {
                    this.gameState.players[peerId].stats.spectatorGuesses = [];
                }
                this.gameState.players[peerId].stats.spectatorGuesses.push({ target: msg.data, round: this.gameState.round || 1 });
            }
            return; // Don't broadcast state just for a spectator guess to save bandwidth
        }
        if (msg.type === 'JOIN') { 
            if (!this.gameState.players[peerId] || this.gameState.phase === 'LOBBY') { 
                // Assign WAITING status if joining mid-game
                if (this.gameState.phase !== 'LOBBY') {
                    msg.data.status = 'WAITING';
                    msg.data.role = 'Spectator';
                }
    ...
    nextRoundOrEnd() {
        ...
        if(this.gameState.phase === 'RESULT') this.startNightPhase();
            ...
            Object.keys(this.gameState.players).forEach(id => { 
                // Keep REMOVED players out when returning to Lobby
                if (this.gameState.players[id].status !== 'REMOVED') {
    ...
    requestAdminAction(actionType) {
        // Safely hide the old modal if it still exists
        const oldModal = document.getElementById('mafia-host-settings-modal');
        if (oldModal) oldModal.classList.add('hidden');
        
        // Safely hide the new Bottom Sheet so the screen is clear for the slider
        const sheet = document.getElementById('mafia-host-bottom-sheet');
        const overlay = document.getElementById('mafia-host-bottom-sheet-overlay');
        if (sheet) { sheet.classList.add('hidden'); sheet.classList.add('translate-y-full'); }
   ...
   handleClientReceivesData(msg) {
        ...
        // --- NEW: Auto-Repair Missing Players ---
        // If the host just arrived and we are missing from their list, re-send our data silently!
        if (!this.isHost && msg.data.phase === 'LOBBY' && this.myId && !msg.data.players[this.myId]) {
            this.sendToHost({ type: 'JOIN', data: this.player });
        }
        // ----------------------------------------
        ...
        // Safely check for old room badges before updating
        const badge = document.getElementById('mafia-room-badge');
        if (badge) badge.classList.remove('hidden');
        
        const codeDisplay = document.getElementById('mafia-display-room-code');
        if (codeDisplay) codeDisplay.innerText = this.roomId;
        
        const fab = document.getElementById('fab-role-reveal');
        const hostFab = document.getElementById('mafia-host-fab');
        
        if(!this.isHost && ['NIGHT', 'DAY', 'VOTE'].includes(this.gameState.phase)) fab.classList.remove('hidden');
        else fab.classList.add('hidden');

        // NEW: Show Host controls during active gameplay
        if(this.isHost && ['NIGHT', 'DAY', 'VOTE'].includes(this.gameState.phase)) {
            if(hostFab) hostFab.classList.remove('hidden');
        } else {
            if(hostFab) hostFab.classList.add('hidden');
        }
        ...
        // --- NEW: Populate Identity Bar ---
        if (!this.isHost && this.myId && this.gameState.players[this.myId]) {
            const me = this.gameState.players[this.myId];
            document.getElementById('mafia-my-avatar').src = me.avatar;
            document.getElementById('mafia-my-name').innerText = me.name;
            document.getElementById('mafia-my-persona').innerText = me.persona;
        }
    },
    ...
    renderLobby() {
        ...
        if (!this.isHost) {
             // Safely check for old welcome messages before updating
             const welcomeMsg = document.getElementById('mafia-lobby-welcome-msg');
             if (welcomeMsg) welcomeMsg.innerText = `Welcome to ${this.gameState.hostName}'s Pol`;
             
             const hostHeader = document.getElementById('mafia-host-header-panel');
    ...
    renderNight() {
        ...
        if (this.isHost) {
            list.innerHTML = ''; // Host always rebuilds to see live lock-ins
...
const myData = this.gameState.players[this.myId];
        if (!myData || myData.status === 'DEAD' || myData.status === 'WAITING' || this.gameState.nightInputs[this.myId]) {
            actionPanel.classList.add('hidden'); lockedPanel.classList.remove('hidden');
            if (myData && myData.status === 'DEAD') {
                // Interactive Spectator Dashboard
                let activePlayersHtml = Object.values(this.gameState.players)
                    .filter(p => p.status === 'ALIVE')
                    .map(p => `<button onclick="MafiaEngine.submitSpectatorGuess('${p.id}')" class="bg-gray-800 border border-gray-600 rounded p-2 m-1 text-xs font-bold text-gray-300 hover:border-blue-500 active:scale-95">${p.name}</button>`)
                    .join('');

                lockedPanel.innerHTML = `
                    <div class="text-4xl text-blue-500 mb-4"><i class="fa-solid fa-ghost"></i></div>
                    <h2 class="text-xl font-black tracking-widest text-blue-400 mb-2">SPECTATOR MODE</h2>
                    <p class="text-gray-400 text-xs uppercase tracking-widest font-bold mb-4">Who gets eliminated next?</p>
                    <div class="flex flex-wrap justify-center max-w-xs mx-auto mb-4">${activePlayersHtml}</div>
                    <p id="spectator-feedback" class="text-green-500 text-xs font-bold h-4"></p>
                `;
            } else if (myData && myData.status === 'WAITING') {
                lockedPanel.innerHTML = `<div class="text-6xl text-blue-500 mb-6"><i class="fa-solid fa-hourglass-half fa-spin"></i></div><h2 class="text-2xl font-black tracking-widest text-blue-500">WAITING</h2><p class="text-gray-500 text-xs mt-2 uppercase tracking-widest font-bold">Joining next round...</p>`;
            }
            return;
        }
        ...
        // --- ANTI-FLASH FIX: Do not rebuild the list if it is already rendered for this player! ---
       ...
       document.getElementById('btn-confirm-night').classList.remove('opacity-50', 'cursor-not-allowed');
        ...
        // Filter: Must be ALIVE. If it is my own ID, I must be the Doctor to see it.
        let aliveTargets = Object.values(this.gameState.players).filter(p => {
            if (p.status !== 'ALIVE') return false; // Hide eliminated players
            if (p.id === this.myId && myData.role !== 'Doctor') return false; // Hide self UNLESS you are the Doctor
            return true;
...
submitSpectatorGuess(targetId) {
        // Send guess to the host to be logged in stats
        this.sendToHost({ type: 'SPECTATOR_GUESS', data: targetId });
        const feedback = document.getElementById('spectator-feedback');
        if (feedback) {
            feedback.innerText = "Prediction locked in!";
...
renderDay(event) {
        ...
        const updateTimer = () => {
            ...
            const now = Date.now();
            // If paused, push the end time forward so the timer effectively freezes
            if (this.gameState.isPaused && this.isHost) {
                this.gameState.dayEndTime += (now - lastFrameTime);
                this.broadcastState('PAUSED');
            }
            lastFrameTime = now;

            if (this.gameState.isPaused) {
                document.getElementById('mafia-morning-announcement').innerHTML = `<h4 class="font-black text-yellow-500 tracking-widest uppercase"><i class="fa-solid fa-pause"></i> GAME PAUSED BY HOST</h4>`;
            }
    ...
    renderVote() {
        ...
        const myData = this.gameState.players[this.myId];
        if (!myData || myData.status === 'DEAD' || myData.status === 'WAITING' || this.gameState.voteInputs[this.myId]) {
            actionPanel.classList.add('hidden'); lockedPanel.classList.remove('hidden');
            if (myData && myData.status === 'DEAD') {
...
list.innerHTML = '';
        this.selectedTargetId = null; // This ensures they CANNOT skip. They must tap a name.
        ...
        // Filter: Must be ALIVE, and CANNOT be yourself.
        let aliveVoteTargets = Object.values(this.gameState.players).filter(p => {
            if (p.status !== 'ALIVE') return false; // Hide eliminated players
            if (p.id === this.myId) return false; // Hide yourself (cannot vote for self)
            return true;
        }).sort(() => Math.random() - 0.5);

        aliveVoteTargets.forEach(p => {
            const li = document.createElement('li');
...
// --- NEW: HOST BOTTOM SHEET LOGIC ---
    toggleHostBottomSheet() {
        const overlay = document.getElementById('mafia-host-bottom-sheet-overlay');
        const sheet = document.getElementById('mafia-host-bottom-sheet');
    ...
    togglePause() {
        ...
        const btn = document.getElementById('btn-mafia-pause');
        
        if (this.gameState.isPaused) {
            btn.classList.replace('bg-blue-900/40', 'bg-yellow-900/40');
            btn.classList.replace('border-blue-600', 'border-yellow-600');
            btn.classList.replace('text-blue-400', 'text-yellow-400');
            btn.innerHTML = `<span>Resume Game</span> <i class="fa-solid fa-play"></i>`;
        } else {
            btn.classList.replace('bg-yellow-900/40', 'bg-blue-900/40');
            btn.classList.replace('border-yellow-600', 'border-blue-600');
            btn.classList.replace('text-yellow-400', 'text-blue-400');
            btn.innerHTML = `<span>Pause Game</span> <i class="fa-solid fa-pause"></i>`;
        }
        this.broadcastState('PAUSE_TOGGLE');
    },

    renderHostManageList() {
        const list = document.getElementById('mafia-host-manage-list');
        if (!list) return;
        list.innerHTML = '';
        
        Object.values(this.gameState.players).forEach(p => {
            const isSittingOut = p.status === 'SITTING_OUT';
            const isDead = p.status === 'DEAD';
            
            list.innerHTML += `
                <li class="flex justify-between items-center bg-gray-900 p-2 rounded-lg border border-gray-700">
                    <div>
                        <span class="text-sm font-bold ${isDead ? 'text-red-500 line-through' : 'text-white'} block truncate max-w-[120px]">${p.name}</span>
                        <span class="text-[10px] text-gray-500 font-bold uppercase">${p.role || 'No Role'}</span>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="MafiaEngine.togglePlayerStatus('${p.id}')" class="text-[10px] px-2 py-1 rounded font-bold uppercase tracking-wider ${isSittingOut ? 'bg-yellow-900 text-yellow-500' : 'bg-gray-700 text-gray-300'}" ${isDead ? 'disabled opacity-50' : ''}>
                            ${isSittingOut ? 'Sit Out' : 'Active'}
                        </button>
                        <button onclick="MafiaEngine.kickPlayer('${p.id}')" class="text-xs bg-red-900/50 text-red-500 px-3 py-1 rounded font-bold border border-red-800"><i class="fa-solid fa-ban"></i></button>
                    </div>
                </li>
            `;
        });
    ...
    togglePlayerStatus(id) {
        if (this.gameState.players[id].status === 'DEAD') return;
        
        if (this.gameState.players[id].status === 'SITTING_OUT') {
            this.gameState.players[id].status = 'ALIVE';
        } else {
            this.gameState.players[id].status = 'SITTING_OUT';
        }
        
        this.renderHostManageList();
        this.broadcastState();
        
        // Auto-trigger phase progressions if sitting this player out fulfilled the requirements
        if (this.gameState.phase === 'NIGHT') this.checkNightProgress();
        if (this.gameState.phase === 'VOTE') this.checkVoteProgress();
    },

    kickPlayer(id) {
        if(confirm(`Remove ${this.gameState.players[id].name} from the Pol? They will be kicked immediately.`)) {
    ...
    endGameInstantly() {
        this.toggleHostBottomSheet();
        this.endGame('SESSION CONCLUDED', 'The Pol Panchayat was dismissed early by the Host.', 'fa-flag-checkered text-blue-500', 'text-blue-500');
    }
