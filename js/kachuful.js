const KFEngine = {
    ...
    renderBoard() {
       ...
       const btnAction = document.getElementById('kf-btn-action');
        const btnUnlock = document.getElementById('kf-btn-unlock');
...
// Add the unlock method:
 ...
 editRound(idx) {
        ...
        // Remove the points they earned in that round
        this.state.names.forEach(n => {
        ...
        this.state.phase = 'score'; // Start in score phase for easier editing
        
        // IMPORTANT FIX: Map bids directly by name instead of breaking the array shift
        this.state.bids = this.state.players.map(p => rec.data[p.name] ? rec.data[p.name].b : 0);
        this.state.misses = this.state.players.map(p => rec.data[p.name] ? !rec.data[p.name].made : false);
