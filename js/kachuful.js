    ...
    renderBoard() {
        ...
        if (this.isFirstRender || rowsContainer.children.length === 0) {
            ...
            this.state.players.forEach((p, i) => {
                let html = `<div class="kf-player-row" id="kf-row-${i}"><div class="kf-name" id="kf-name-${i}">${p.name}</div>`;
        ...
        this.state.players.forEach((p, i) => {
            ...
            if(statBtn && typeof NetworkEngine !== 'undefined' && NetworkEngine.role === 'host') {
                ...
                if (this.state.phase === 'bid') {
                    ...
                    if (rowEl) { rowEl.onclick = null; rowEl.style.cursor = 'default'; }
                } else {
                    ...
                    // Make the entire row clickable to toggle status
                    if (rowEl) {
                        rowEl.onclick = () => KFEngine.togMiss(i);
