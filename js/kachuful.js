const KFEngine = {
    ...
    renderBoard() {
        ...
        if (this.isFirstRender || rowsContainer.children.length === 0 || (trumpCardEl && rowsContainer.children.length === 1)) {
            ...
            this.state.players.forEach((p, i) => {
                ...
                if (trumpCardEl && trumpCardEl.parentElement === rowsContainer) {
                    trumpCardEl.insertAdjacentHTML('beforebegin', html);
                } else {
        ...
        this.state.players.forEach((p, i) => {
            ...
            if (this.state.phase === 'bid') {
                ...
                if (nameEl) { 
                    nameEl.onclick = null; 
                    nameEl.classList.remove('clickable-name'); 
                }
                ...
                if (statBtn && typeof NetworkEngine !== 'undefined' && NetworkEngine.role === 'host') {
                    ...
                    if (this.lastToggledMiss === i) {
                        statBtn.classList.remove('animate-stamp');
                        void statBtn.offsetWidth;
                        statBtn.classList.add('animate-stamp');
                    }
                ...
                if (specWrapper && typeof NetworkEngine !== 'undefined' && NetworkEngine.role === 'spectator') {
                    ...
                    if (specStamp) {
                        ...
                        if (this.lastToggledMiss === i) {
                            specStamp.classList.remove('animate-stamp');
                            void specStamp.offsetWidth;
                            specStamp.classList.add('animate-stamp');
                        }
    ...
    triggerTextPing(idx, dir) {
        ...
        [nameEl, hostBid, specBid].forEach(el => {
            if (el) {
                el.classList.remove('text-ping');
                void el.offsetWidth; 
                el.classList.add('text-ping');
            }
        });
    ...
    handleAction() {
        if (this.state.phase === 'bid') {
            ...
            activePanel.classList.remove('phase-locked');
            void activePanel.offsetWidth; 
            activePanel.classList.add('phase-locked');
            ...
            this.state.players.forEach((p, i) => {
                ...
                if (pts > 0) {
                    ...
                    if (nameEl) {
                        ...
                        particle.className = 'floating-points';
                        particle.innerText = `+${pts}`;
    ...
    receiveSurgicalUpdate(newState) {
        ...
        if (this.state.phase === 'score' && newState.phase === 'score') {
            newState.misses.forEach((m, i) => {
                if (m !== this.state.misses[i]) {
                    this.lastToggledMiss = i;
                }
            });
        }
        ...
        if (newState.phase === 'bid') {
            newState.bids.forEach((newBid, i) => {
                if (newBid !== this.state.bids[i]) {
                    const dir = newBid - this.state.bids[i];
                    this.triggerTextPing(i, dir);
                }
            });
        }

        const oldTot = this.state.bids.reduce((a,b)=>a+b, 0);
        const newTot = newState.bids.reduce((a,b)=>a+b, 0);
