...
// Inside hub.js
    init() {
        ...
        if (raw) { 
            try { 
                ...
                const now = Date.now();
                // CHANGED TO 5 HOURS (18,000,000 ms)
                if (savedState.lastActionTime && (now - savedState.lastActionTime > 18000000)) {
        ...
        // STREAMLINED QR FLOW: Hide Host UI immediately if joining via link/QR
        if (roomParam) {
            const joinInput = document.getElementById('join-room-code');
            if (joinInput) joinInput.value = roomParam.toUpperCase();
            
            document.getElementById('hub-host-controls').style.display = 'none';
            document.getElementById('btn-scan-qr').style.display = 'none';
            document.getElementById('join-player-name').focus();
        } else if (this.state.pendingGame) {
            // Prompt to resume if an active local session exists and they didn't join via QR
            document.getElementById('hub-resume-prompt').classList.remove('hidden');
        }
    ...
    // New Helper Methods for the Resume Prompt
    clearActiveSession() {
        ...
        document.getElementById('hub-resume-prompt').classList.add('hidden');
        this.pingAction();
    },
