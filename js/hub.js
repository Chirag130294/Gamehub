const GujaratLoader = {
    ...
    update() {
        const titleEl = document.getElementById('gujarat-loader-title');
        const descEl = document.getElementById('gujarat-loader-desc');
        const bgEl = document.getElementById('gujarat-loader-bg');
        if(!titleEl || !descEl || !bgEl) return;
        
        const currentPlace = this.places[this.idx];
        const img = new Image();
        
        // --- NEW: Hardened Asset Loading ---
        img.onload = () => {
            titleEl.style.opacity = 0; descEl.style.opacity = 0; bgEl.style.opacity = 0;
            
            setTimeout(() => {
                titleEl.innerText = currentPlace.t;
                descEl.innerText = currentPlace.d;
                bgEl.style.backgroundImage = `url('${currentPlace.img}')`;
                
                titleEl.style.opacity = 1; descEl.style.opacity = 1; bgEl.style.opacity = 1; 
            }, 1000); 
        };

        // Fallback: If image is missing, just load the text with a dark gradient
        img.onerror = () => {
            titleEl.style.opacity = 0; descEl.style.opacity = 0; bgEl.style.opacity = 0;
            setTimeout(() => {
                titleEl.innerText = currentPlace.t;
                descEl.innerText = currentPlace.d;
                bgEl.style.backgroundImage = `linear-gradient(to bottom, #1a202c, #0d1117)`; // Safe fallback
                titleEl.style.opacity = 1; descEl.style.opacity = 1; bgEl.style.opacity = 1; 
            }, 1000);
        };

        img.src = currentPlace.img; // Trigger the load
        // -----------------------------------
    },
...
initLobby(gameId) {
        ...
        if (gameId === 'mafia') {
            ...
            // Auto-fill Mafia's host input so they don't type it twice
            const mafiaHostInput = document.getElementById('mafia-host-name');
            if (mafiaHostInput && this.state.hostName) mafiaHostInput.value = this.state.hostName;
            return;
        }
    ...
    continueToSetup() { this.switchView(this.state.pendingGame); },
    ...
    showRoomPanel() {
        ...
        if (typeof NetworkEngine !== 'undefined') {
            document.getElementById('room-code-display').innerText = `ROOM: ${NetworkEngine.roomCode}`;
    ...
    startQRScanner() {
        ...
        this.html5QrcodeScanner.render((decodedText) => {
            document.getElementById('join-room-code').value = decodedText;
